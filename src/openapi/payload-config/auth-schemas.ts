...
export const getAuthSchemas = (
  ...
  if (
    config.collections.some((collection) => collection.auth.maxLoginAttempts)
  ) {
    requestBodies["unlockRequest"] = createRequestBody(unlock);
  }

  if (options.include.passwordRecovery) {
    requestBodies["passwordForgottenRequest"] =
      createRequestBody(passwordForgotten);
