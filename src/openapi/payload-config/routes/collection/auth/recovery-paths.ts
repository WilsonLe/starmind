...
export const getPasswordRecoveryPaths = (
  ...
  return {
    [`/${collection.slug}/forgot-password`]: {
      post: {
        ...
        description:
          "Entry point for password reset workflow. Sends password reset email.",
        tags: ["auth"],
    ...
    [`/${collection.slug}/reset-password`]: {
      post: {
        ...
        responses: {
          "200": createRef(
            `${getSingularSchemaName(collection)}PasswordResetResponse`,
            "responses",
          ),
        },
      },
