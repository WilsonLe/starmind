...
const getTags = (slug: string, payloadConfig: SanitizedConfig) =>
  payloadConfig.globals?.find((global) => global.slug === slug)
    ? [`global ${slug}`]
    : [slug];
...
export const createVersionRoutes = async (
  ...
  if (!config.versions || !isRouteAvailable(config, "readVersions"))
  ...
  const components: OpenAPIV3.ComponentsObject = {
    schemas: {
      ...
      [`${schemaName}Versions`]: createPaginatedDocumentSchema(
        `${schemaName}Version`,
        `${singular} versions`,
      ),
    },
    responses: {
      [`${schemaName}VersionResponse`]: createResponse(
        "ok",
        `${schemaName}Version`,
      ),
      [`${schemaName}VersionsResponse`]: createResponse(
        "ok",
        `${schemaName}Versions`,
      ),
    },
