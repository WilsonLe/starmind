...
const getText = (
  ...
  return kind === "singular"
    ? collection.slug
    : `${getText(collection, "singular")}s`;
};

export const getSingularSchemaName = (
  collection: SanitizedCollectionConfig | SanitizedGlobalConfig,
): string => getText(collection, "singular");

export const getPluralSchemaName = (
  collection: SanitizedCollectionConfig | SanitizedGlobalConfig,
): string => getText(collection, "plural");
