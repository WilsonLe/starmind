...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`agents\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`created_by_users_id\` integer,
  	\`updated_by_users_id\` integer,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`_status\` text DEFAULT 'draft',
  	...
  	\`api_key\` text,
  	\`api_key_index\` text,
  	FOREIGN KEY (\`created_by_users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`updated_by_users_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`agents_created_by_users_idx\` ON \`agents\` (\`created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`agents_updated_by_users_idx\` ON \`agents\` (\`updated_by_users_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_agents_v\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`parent_id\` integer,
  	\`version_created_by_users_id\` integer,
  	\`version_updated_by_users_id\` integer,
  	\`version_updated_at\` text,
  	\`version_created_at\` text,
  	\`version__status\` text DEFAULT 'draft',
  	...
  	FOREIGN KEY (\`version_created_by_users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_updated_by_users_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_agents_v_parent_idx\` ON \`_agents_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_agents_v_version_version_created_by_users_idx\` ON \`_agents_v\` (\`version_created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`_agents_v_version_version_updated_by_users_idx\` ON \`_agents_v\` (\`version_updated_by_users_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`media_created_by_users_idx\` ON \`media\` (\`created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`media_updated_by_users_idx\` ON \`media\` (\`updated_by_users_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_media_v_version_version_created_by_users_idx\` ON \`_media_v\` (\`version_created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`_media_v_version_version_updated_by_users_idx\` ON \`_media_v\` (\`version_updated_by_users_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`notes\` (
  	...
  	\`created_by_users_id\` integer,
  	\`updated_by_users_id\` integer,
  	\`created_by_agents_id\` integer,
  	\`updated_by_agents_id\` integer,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`_status\` text DEFAULT 'draft',
  	FOREIGN KEY (\`created_by_users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`updated_by_users_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`created_by_agents_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`updated_by_agents_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`notes_created_by_users_idx\` ON \`notes\` (\`created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`notes_updated_by_users_idx\` ON \`notes\` (\`updated_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`notes_created_by_agents_idx\` ON \`notes\` (\`created_by_agents_id\`);`)
  await db.run(sql`CREATE INDEX \`notes_updated_by_agents_idx\` ON \`notes\` (\`updated_by_agents_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`_notes_v\` (
  	...
  	\`version_created_by_users_id\` integer,
  	\`version_updated_by_users_id\` integer,
  	\`version_created_by_agents_id\` integer,
  	\`version_updated_by_agents_id\` integer,
  	\`version_updated_at\` text,
  	\`version_created_at\` text,
  	\`version__status\` text DEFAULT 'draft',
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	...
  	FOREIGN KEY (\`version_created_by_users_id\`) REFERENCES \`users\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_updated_by_users_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_created_by_agents_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`version_updated_by_agents_id\`) REFERENCES \`agents\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`_notes_v_parent_idx\` ON \`_notes_v\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`_notes_v_version_version_created_by_users_idx\` ON \`_notes_v\` (\`version_created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`_notes_v_version_version_updated_by_users_idx\` ON \`_notes_v\` (\`version_updated_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`_notes_v_version_version_created_by_agents_idx\` ON \`_notes_v\` (\`version_created_by_agents_id\`);`)
  await db.run(sql`CREATE INDEX \`_notes_v_version_version_updated_by_agents_idx\` ON \`_notes_v\` (\`version_updated_by_agents_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`note_tags_created_by_users_idx\` ON \`note_tags\` (\`created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`note_tags_updated_by_users_idx\` ON \`note_tags\` (\`updated_by_users_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_note_tags_v_version_version_created_by_users_idx\` ON \`_note_tags_v\` (\`version_created_by_users_id\`);`)
  await db.run(sql`CREATE INDEX \`_note_tags_v_version_version_updated_by_users_idx\` ON \`_note_tags_v\` (\`version_updated_by_users_id\`);`)
